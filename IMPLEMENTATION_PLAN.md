...
**Статус:** первичный снимок фактического рабочего дерева записан в
`docs/inventory/`. Перед закрытием gate требуется машинная проверка покрытия всех
исходных Python-файлов и тестов и решение, как зафиксировать dirty snapshot.
