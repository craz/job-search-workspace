...
- команды `bootstrap`, `doctor`, `dev`, `test`, `backup`, `restore`;
- ADR по multirepo, PostgreSQL и межсервисной интеграции.

**Gate:** workspace можно клонировать отдельно и по документации понять устройство
системы и способ работы с репозиториями.
...
Следующая выполняемая задача — полная инвентаризация
`/data/Projects/job_search` и создание migration map. Реализация Core начинается
только после назначения владельца каждому исходному модулю, сценарию и набору
данных.
