...
Машинный gate запускается командой `make inventory-check`. Решающее дерево в
`scripts/check_inventory.py` сканирует фактический архив и назначает каждому
Python-файлу и operational script ровно одного владельца. `deferred-local`
означает явно отложенный локальный scope, а не ничейный файл.
Проверенный dirty snapshot закреплён агрегированным hash путей и содержимого в
`source-code.lock.json`; изменение, добавление, удаление или переименование
исходного code-файла делает gate красным до повторной инвентаризации.
