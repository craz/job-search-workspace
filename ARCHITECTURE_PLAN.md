...
- общий manifest-репозиторий связывает проекты без Git submodules и subtree;
...
- `repos.yaml` со списком Git URL и совместимых версий;
...
`bootstrap` клонирует недостающие репозитории как соседние каталоги. Обычная
работа с workspace не изменяет Git-состояние дочерних репозиториев автоматически.
